...
import pytest
from custom_datetime import MyDatetime  

def test_datetime_creation_no_args():
    datetime_obj_no_args = MyDatetime()
    assert isinstance(datetime_obj_no_args, MyDatetime)
    assert datetime_obj_no_args.year == 2023
    assert datetime_obj_no_args.month == 11
    assert datetime_obj_no_args.day == 28
    assert datetime_obj_no_args.hour == 15
    assert datetime_obj_no_args.minute == 30
    assert datetime_obj_no_args.second == 45
